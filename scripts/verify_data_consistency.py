"""Verify consistency across parquet_shards, dedup (simhashes), and BPB data."""
...
def main():
    ...
    print("Loading bpb books...")
    bpb = load_jsonl_books(BPB_DIR, ".bpb.jsonl", "BPBs")
    print(f"  {len(bpb):,} books")
    ...
    if mismatches:
        ...
        for i, (bc, (p, s, x)) in enumerate(sorted(mismatches.items())):
            if i >= 20:
                ...
                break
            print(f"  {bc}: parquet={p}, simhash={s}, bpb={x}")
    else:
