...
Requires local model weights and data; auto-skips when they are unavailable.
...
requires_data = pytest.mark.skipif(not DATA_FILE.exists(), reason="Local data not available")
...
class TestBPBBatched:
    ...
    def _compute_batched(self, paragraphs, model, tokenizer, device, batch_size=8):
        ...
        return compute_bpb_batched(paragraphs, model, tokenizer, device, batch_size=batch_size)
