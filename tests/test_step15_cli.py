...
class TestStep15CLI:
        ...
        books = [
            {
                "barcode_src": "book1",
                "language_gen": "eng",
                "annotated_frontmatter": "<header>...</header>",
    ...
    def test_keep_flags_retain_fields(self, tmp_path: Path):
        """--keep-sentences/--keep-indices should retain the fields they name."""
        input_file = tmp_path / "input.jsonl"
        output_file = tmp_path / "output.jsonl"
        ...
        output_book = json.loads(output_file.read_text().strip())
        assert output_book["middlematter_sentences"] == ["A.", "B."]
        assert output_book["subtopic_paragraph_start_indices"] == [0, 1]
        assert output_book["subtopic_section_start_indices"] == [0]
