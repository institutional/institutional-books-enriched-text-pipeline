...
def main(
    ...
    incomplete_path = output_file.with_name(output_file.stem + ".incomplete.jsonl")

    processed_book_ids: set[str] = set()
    if resume:
        processed_book_ids = load_processed_book_ids(progress_path)
        if processed_book_ids:
            logger.info(f"Resuming: found {len(processed_book_ids)} previously processed books")
        else:
        ...
        with open_jsonl(input_file, "r") as f_in:
            for line in f_in:
                ...
                try:
                    ...
                    incomplete_file.write(json.dumps({"book_id": book_id, "error": str(e)}) + "\n")
