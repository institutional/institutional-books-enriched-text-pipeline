...
4. Annotate         (per shard, reads only its own small lookup)

clusters.json is streamed exactly ONCE. For every cluster {rep: [member, ...]}
each member doc_id ("<barcode>.<para_idx>") is routed to the lookup file of the
shard that owns its barcode, written as a TSV line:
...
This replaces the old approach in which dedup_annotate re-streamed the entire
(multi-GB) clusters.json once per shard -- turning ~N_shards full scans into 1.
...
# the start of the line -- far cheaper than json.loads on a multi-KB book object.
...
def index_shard_file(path: Path) -> tuple[str, list[str]]:
    ...
    Extracts only barcode_src via regex (the first field of each line), avoiding
    a full JSON parse of every book.
    """
    barcodes: list[str] = []
    with open(path) as f:
        for line in f:
...
def main(
    ...
    logger.info(
        f"Indexed {len(barcode_to_shard):,} barcodes across {len(shard_files)} shards"
    )
    ...
    if missing:
        logger.warning(
            f"{missing:,} cluster members had a barcode not present in any shard "
            "(skipped)"
        )
