...
Handles primary pipeline steps.
...
def process_shard(
    ...
    if resume:
        ...
        if processed_barcodes:
            logger.info(f"Resuming: found {len(processed_barcodes)} previously processed books")
        else:
...
def reprocess_incomplete(
    ...
    if complete_progress.exists():
        ...
        with (
            open_jsonl(complete_path, "r") as src,
            open(complete_progress, "w", encoding="utf-8") as dst,
        ):
            for line in src:
                dst.write(line)
    ...
    try:
        for book in incomplete_books:
            ...
            logger.info(
                f"Processing book {book_id} from {remaining_steps[0]} (last completed: {last_step})"
            )
