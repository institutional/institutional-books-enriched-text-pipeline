...
from utils.jsonl_io import load_bpb_map, open_jsonl

# Base fields always kept in final output (step15). Intermediate fields
# (sentences, paragraph/section indices) are added on top of this set only when
...
def step14_add_metadata_book(
    ...
    book["metadata"] = {**text_stats, **bpb_stats}
    return book
    ...
    try:
        for step_name in steps:
            if step_name == "step13_annotate":
                current_book = step13_annotate_book(current_book, em_classifier, bpb_map)
            elif step_name == "step14_add_metadata":
                current_book = step14_add_metadata_book(current_book, bpb_map)
        ...
        with open_jsonl(input_file, "r") as f:
            for line_num, line in enumerate(f, 1):
                ...
                result_book, error_msg = postprocess_book(
                    book, steps, em_classifier, bpb_map, keep_sentences, keep_indices
                )
...
@click.option(
    ...
    default=False,
    help="Keep middlematter_sentences in final output (default: drop)",
)
@click.option(
    "--keep-indices/--no-keep-indices",
    default=False,
    help="Keep paragraph/section indices in final output (default: drop)",
)
@click.option(
    "--resume",
    is_flag=True,
